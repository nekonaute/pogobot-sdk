...
#define CSR_SPI_CS_SPI_CS_N_RAM_CS_N_OFFSET 2
#define CSR_SPI_CS_SPI_CS_N_RAM_CS_N_SIZE 1
static inline uint32_t spi_cs_spi_cs_n_ram_cs_n_extract(uint32_t oldword) {
	uint32_t mask = ((uint32_t)(1 << 1)-1);
	return ( (oldword >> 2) & mask );
}
static inline uint32_t spi_cs_spi_cs_n_ram_cs_n_read(void) {
	uint32_t word = spi_cs_spi_cs_n_read();
	return spi_cs_spi_cs_n_ram_cs_n_extract(word);
}
static inline uint32_t spi_cs_spi_cs_n_ram_cs_n_replace(uint32_t oldword, uint32_t plain_value) {
	uint32_t mask = ((uint32_t)(1 << 1)-1);
	return (oldword & (~(mask << 2))) | (mask & plain_value)<< 2 ;
}
static inline void spi_cs_spi_cs_n_ram_cs_n_write(uint32_t plain_value) {
	uint32_t oldword = spi_cs_spi_cs_n_read();
	uint32_t newword = spi_cs_spi_cs_n_ram_cs_n_replace(oldword, plain_value);
	spi_cs_spi_cs_n_write(newword);
}
